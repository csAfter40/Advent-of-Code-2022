...
class Sand():
    ...
    def move(self) -> Tuple[int]:
        ...
        """
        i, j = self.location
        if isinstance(self.cave.map[i+1][j], Air):
            self.location = (i+1, j)
...
class App():
    ...
    def create_map(self) -> List[List]:
        ...
        offset = (offset[0], offset[1]-dimensions[0])
        dimensions = (dimensions[0]+2, dimensions[1]+2*dimensions[0])
        ...
        for i, item in enumerate(map[-1]):
            map[-1][i] = Rock(self.cave)
        return map
